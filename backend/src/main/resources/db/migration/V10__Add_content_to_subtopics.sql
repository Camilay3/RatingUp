...
UPDATE subtopics SET content = 'Introduzindo agora o funcionamento do tabuleiro e das peças de forma mais detalhada, é importante se familiarizar com a estrutura do tabuleiro inicialmente.

O tabuleiro é onde ocorre todo o andamento da partida. Ele tem um formato perfeitamente quadrado e é dividido em uma malha 8x8, totalizando 64 quadrados pequenos, que na linguagem do xadrez são chamados de casas.
...
Contudo, independentemente do na linguagem oficial do xadrez as casas sempre são referidas como casas brancas e pretas ou claras e escuras. Esse padrão alternado não serve apenas para estética, também é útil para melhorar a noção de espaço do tabuleiro para cada jogador.'
...
Como mencionado antes, o Rei possui valor infinito porque a captura significa o fim da partida, a situação de troca de peças (capturar uma peça sabendo que uma de suas peças será capturada em seguida) ou sacrifício para posteriormente continuar a partida basicamente é impossível de ocorrer com o Rei.
...
UPDATE subtopics SET content = 'Começando esse capítulo de regras especiais, é importante entender que algumas jogadas do xadrez funcionam de maneira diferente dos movimentos normais das peças. Essas jogadas existem para trazer mais estratégia para a partida e, quando usadas corretamente, podem mudar completamente o rumo do jogo. A primeira delas é o roque pequeno.

O roque pequeno é um movimento especial que envolve o rei e a torre do lado direito do tabuleiro. Durante essa jogada, o rei anda duas casas em direção à torre, enquanto a torre passa por cima dele e fica posicionada logo ao seu lado. Esse movimento é muito importante porque consegue cumprir duas funções ao mesmo tempo: proteger o rei e desenvolver a torre para uma posição mais ativa.
...
UPDATE subtopics SET content = 'No subtópico anterior foi explicado o funcionamento do roque pequeno. Agora será mostrado o roque grande, que segue uma lógica parecida, mas acontece do lado esquerdo do tabuleiro e possui algumas diferenças estratégicas importantes.

O roque grande também é um movimento especial envolvendo o rei e uma torre. Nesse caso, é utilizada a torre do lado esquerdo. O rei anda duas casas em direção à torre, e a torre pula para o lado do rei, ficando ao lado dele após o movimento. Apesar de parecer semelhante ao roque pequeno, o resultado final da posição costuma gerar ideias diferentes no jogo.
...
UPDATE subtopics SET content = 'Outra regra especial muito importante no xadrez é o en passant. Esse é um movimento que costuma causar dúvida em iniciantes, justamente porque funciona de maneira diferente das capturas normais dos peões. Porém, entendendo sua lógica, ele se torna bastante simples.
...
UPDATE subtopics SET content = 'Encerrando o capítulo de regras especiais, temos a promoção do peão, uma das jogadas mais importantes do xadrez. Esse recurso mostra como até mesmo a peça considerada mais simples do jogo pode se tornar extremamente poderosa.

A promoção acontece quando um peão consegue atravessar todo o tabuleiro e chega até a última fileira do lado adversário. Nesse momento, ele deixa de ser um peão e pode ser transformado em outra peça. Na maioria das vezes, os jogadores escolhem promover para dama, pois ela é a peça mais forte do jogo e possui grande capacidade ofensiva.
...
UPDATE subtopics SET content = 'Começando o capítulo sobre abertura, é importante entender que os primeiros movimentos de uma partida têm enorme impacto no restante do jogo. Uma abertura bem feita ajuda o jogador a desenvolver suas peças de forma organizada e segura. O primeiro princípio fundamental é o controle do centro.

O centro do tabuleiro é formado pelas casas mais importantes da partida, pois a partir delas as peças conseguem se mover com maior liberdade. Quando um jogador controla o centro, suas peças normalmente possuem mais espaço para atacar, defender e criar ameaças.
...
UPDATE subtopics SET content = 'Após entender a importância do centro, o próximo passo é aprender a desenvolver corretamente as peças. Desenvolvimento significa tirar as peças de suas posições iniciais e colocá-las em casas úteis e ativas.
...
UPDATE subtopics SET content = 'No subtópico anterior foi mostrado como desenvolver as peças corretamente. Porém, além de desenvolver as peças, também é essencial proteger o rei desde os primeiros movimentos da partida.

O rei é a peça mais importante do jogo. Se ele estiver exposto, ataques rápidos podem surgir e a partida pode acabar em poucos lances. Por isso, a segurança do rei deve ser tratada como prioridade durante toda a abertura.
...
UPDATE subtopics SET content = 'Dando continuidade aos princípios básicos de abertura, é importante entender que tempo é algo muito valioso no xadrez. Cada jogada utilizada deve ajudar no desenvolvimento da posição. Por isso, mover a mesma peça várias vezes logo no início da partida normalmente não é uma boa ideia.
...
UPDATE subtopics SET content = 'Após desenvolver as peças menores e proteger o rei, outro objetivo importante da abertura é conectar as torres. Esse conceito é muito importante porque mostra quando a posição está ficando organizada e pronta para entrar no meio-jogo.

Conectar as torres significa deixar o caminho livre entre elas. Isso normalmente acontece depois que cavalos, bispos e dama saem das casas iniciais. Quando as torres conseguem “se enxergar” horizontalmente, dizemos que estão conectadas.
...
UPDATE subtopics SET content = 'Encerrando o capítulo de abertura, é importante entender que existem muitas aberturas conhecidas no xadrez. Essas aberturas foram estudadas durante muitos anos e ajudam os jogadores a começarem a partida de maneira organizada e eficiente.
...
UPDATE subtopics SET content = 'Começando o capítulo sobre tática, é importante entender que a tática representa combinações e jogadas que criam vantagens imediatas durante a partida. Muitas vezes, uma boa tática pode decidir o jogo rapidamente. Uma das mais conhecidas é o ataque duplo, também chamado de garfo.

O ataque duplo acontece quando uma peça ameaça duas peças adversárias ao mesmo tempo. Isso obriga o adversário a escolher qual delas salvar, normalmente perdendo a outra peça.
...
UPDATE subtopics SET content = 'No subtópico anterior foi apresentado o ataque duplo. Agora será mostrada outra tática muito importante: a cravada.

A cravada acontece quando uma peça fica impedida de se mover porque está protegendo outra peça mais importante atrás dela. Se a peça cravada sair do lugar, a peça mais valiosa ficará vulnerável à captura.
...
UPDATE subtopics SET content = 'Outra tática bastante importante é o ataque descoberto. Esse tipo de jogada acontece quando uma peça se move e revela o ataque de outra peça que estava bloqueada atrás dela.
...
UPDATE subtopics SET content = 'No subtópico anterior foi explicado o conceito de descoberto. Agora será mostrado o espeto, uma das formas mais perigosas dessa tática.

O espeto acontece quando o ataque revelado é um xeque contra o rei adversário. Isso torna a jogada extremamente forte, pois o rei obrigatoriamente precisa ser defendido.
...
Muitas vezes, o espeto permite ganhar peças importantes ou até finalizar a partida rapidamente. Por isso, jogadores experientes utilizam bastante esse recurso em combinações ofensivas.
...
UPDATE subtopics SET content = 'Encerrando o capítulo de tática, temos um método muito útil para ajudar na tomada de decisões durante a partida: o CCT, sigla para Checks, Captures e Threats, que significa xeques, capturas e ameaças.
...
UPDATE subtopics SET content = 'Começando o capítulo sobre meio-jogo, é importante entender que essa é a fase mais dinâmica e complexa de uma partida de xadrez. Depois que a abertura termina e as peças já estão desenvolvidas, o jogo entra em uma etapa onde os planos e estratégias passam a ter ainda mais importância.
...
UPDATE subtopics SET content = 'No subtópico anterior foi explicado o que caracteriza o meio-jogo. Agora será mostrado um dos conceitos mais importantes dessa fase: a criação de planos.

No xadrez, jogar apenas respondendo ao adversário sem um objetivo definido normalmente leva a posições ruins. Durante o meio-jogo, cada movimento deve fazer parte de uma ideia maior. Essas ideias são chamadas de planos.
...
Aprender a atacar o rei ajuda o jogador a desenvolver criatividade, visão tática e capacidade de coordenação entre as peças.'
WHERE title = 'Ataque ao rei'
  AND chapter_id = (SELECT id FROM chapters WHERE display_order = 8);

UPDATE subtopics SET content = 'Outro conceito muito importante no meio-jogo é o de casas fracas. Esse é um tema estratégico que influencia bastante o posicionamento das peças durante a partida.

Casas fracas são casas que não podem mais ser protegidas por peões. Isso normalmente acontece depois que determinados peões avançam ou são trocados. Como os peões não conseguem voltar para trás, essas fraquezas costumam ser permanentes.
...
UPDATE subtopics SET content = 'No meio-jogo, algumas regiões do tabuleiro se tornam especialmente importantes para as torres. Entre elas estão as colunas abertas e semi-abertas.

Uma coluna aberta é uma coluna sem peões de nenhum dos lados. Já uma coluna semi-aberta possui peões de apenas um jogador. Essas colunas funcionam como caminhos livres para as torres e damas atuarem com maior facilidade.
...
Aprender esse conceito ajuda o jogador a criar posições mais ativas e organizadas, aumentando bastante a eficiência das próprias peças.'
WHERE title = 'Peças boas vs ruins'
  AND chapter_id = (SELECT id FROM chapters WHERE display_order = 8);

UPDATE subtopics SET content = 'Encerrando o capítulo sobre meio-jogo, temos um dos conceitos mais importantes do xadrez: a coordenação entre as peças.

No xadrez, peças trabalhando juntas são muito mais fortes do que peças agindo isoladamente. Quando existe coordenação, as peças conseguem se defender mutuamente, criar ameaças combinadas e controlar mais regiões do tabuleiro ao mesmo tempo.
...
UPDATE subtopics SET content = 'Começando o capítulo sobre finais, é importante entender que essa é a fase da partida onde restam poucas peças no tabuleiro. Depois de várias trocas ocorridas durante o meio-jogo, o jogo entra em uma etapa mais técnica, onde cada movimento possui enorme importância.
...
UPDATE subtopics SET content = 'No subtópico anterior foi explicado o que caracteriza a fase final de uma partida. Agora é importante entender que existem diferentes tipos de finais, e cada um deles exige conhecimentos específicos e maneiras diferentes de jogar.
...
Com isso, o jogador começa a perceber que o xadrez é um jogo conectado em todas as fases, onde as decisões tomadas antes influenciam diretamente o resultado final da partida.'
WHERE title = 'Tipos de final'
  AND chapter_id = (SELECT id FROM chapters WHERE display_order = 9);

UPDATE subtopics SET content = 'Um dos conceitos mais importantes dos finais é a oposição de reis. Esse tema parece simples à primeira vista, mas possui enorme impacto principalmente nos finais de peões.

A oposição acontece quando os dois reis ficam frente a frente, separados por apenas uma casa. Nessa situação, o jogador que não possui a vez de jogar geralmente fica em desvantagem, porque será obrigado a ceder espaço ao adversário.
...
UPDATE subtopics SET content = 'Entre todos os finais existentes, um dos mais importantes de aprender é o final de rei e peão contra rei. Esse tipo de posição aparece com muita frequência e ensina vários conceitos fundamentais dos finais.

O objetivo desse final é simples: utilizar o rei para apoiar o avanço do peão até a promoção. Entretanto, mesmo sendo aparentemente fácil, muitas posições exigem bastante precisão.
...
UPDATE subtopics SET content = 'Outro conceito muito útil nos finais é a regra do quadrado do peão. Essa técnica ajuda o jogador a calcular rapidamente se um rei conseguirá alcançar um peão antes da promoção.
...
Com isso, será possível tomar decisões mais rápidas e precisas em situações onde cada lance pode decidir a partida.'
WHERE title = 'Regra do quadrado do peão'
  AND chapter_id = (SELECT id FROM chapters WHERE display_order = 9);

UPDATE subtopics SET content = 'Encerrando o capítulo de finais, é essencial aprender alguns padrões básicos de xeque-mate. Esses padrões aparecem com frequência e ajudam o jogador a finalizar partidas de maneira segura e eficiente.
...
Aprender esses padrões também ajuda na defesa, já que o jogador passa a reconhecer ameaças perigosas com mais facilidade.
Mate da escadinha

O mate da escadinha é um dos padrões mais simples e educativos do xadrez. Ele normalmente é realizado com duas torres ou com torre e dama trabalhando juntas.
...
Treinar esse padrão melhora bastante a noção de coordenação e ajuda iniciantes a desenvolverem mais segurança durante finais vencedores.'
